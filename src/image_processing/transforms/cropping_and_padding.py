...
class CropPad(YuccaTransform):
    ...
    def generate_3D_case_from_3D(self, image, image_properties, label, target_image_shape, target_label_shape, **pad_kwargs):
        unique_lesion_maps = image_properties["unique_lesion_maps"]
        ...
        if len(image_properties["foreground_locations"]) == 0 or np.random.uniform() >= self.p_oversample_foreground:
            ...
            for d in range(3): 
                if image.shape[d + 1] < self.patch_size[d]:
                    ...
                    crop_start_idx += [
                        np.random.randint(
                            max(0, (location[d] - self.patch_size[d] // 2)), # 30
        ...
        unique_lesions = {}
        for lesion in unique_lesion_maps:
            ...
            temp[
                ...
                ((pad_lb_x, pad_ub_x), (pad_lb_y, pad_ub_y), (pad_lb_z, pad_ub_z))
            )
            if np.max(temp) > 0:
                if lesion['location_type'] in unique_lesions:
                    unique_lesions[lesion['location_type']] += 1
                else:
                    unique_lesions[lesion['location_type']] = 1
        return image_out, label_out, unique_lesions
    ...
    def __call__(self, packed_data_dict=None, image_properties=None, **unpacked_data_dict):
        ...
        data_dict[self.data_key], data_dict[self.label_key], data_dict['uniques'] = self.__croppad__(
