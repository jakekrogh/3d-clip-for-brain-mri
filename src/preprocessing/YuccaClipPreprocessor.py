...
from utils.globals import ID_TO_LABEL, course_pd
import pandas as pd


class YuccaClipPreprocessor(object):
    """
    ...
    def __init__(self, plans_path, task=None, threads=None, disable_sanity_checks=False, enable_cc_analysis=False):
        ...
        self.target_spacing = []
        self.clinical_pd = course_pd
        
    def get_gender(self,id):        
        row = self.clinical_pd[self.clinical_pd.eq(int(id)).any(axis=1)].head(1)
        return row['Gender'].values[0]

    def get_age(self,id):
        print(id)
        row = self.clinical_pd[self.clinical_pd.eq(int(id)).any(axis=1)].head(1)
        return row['Age at Diagnosis'].values[0]
    ...
    def _preprocess_train_subject(self, subject_id, label_exists: bool, preprocess_label: bool):
        image_props = {}
        id = (subject_id[-5:][:-2])
        image_props['age'] = self.get_age(id)
        image_props['gender'] = self.get_gender(id)
        ...
        if label_exists and preprocess_label:
            image_props["foreground_locations"], image_props["label_cc_n"], image_props["label_cc_sizes"], image_props['unique_lesion_maps'] = self.analyze_label(
                label=label 
            )
    ...
    def preprocess_case_for_inference(self, images: list | tuple, patch_size: tuple, sliding_window_prediction: bool = True, is_label = False):
        """
        ...
        if is_label:
            images[0] = resize(images[0], output_shape=resample_target_size, order=0, anti_aliasing=False)
        else:
            images = self.resample_and_normalize_case(
                images=images,
                target_size=resample_target_size,
        ...
        if is_label:
            image_properties["foreground_locations"], image_properties["label_cc_n"], image_properties["label_cc_sizes"], image_properties['unique_lesion_maps'] = self.analyze_label(
                label=images[0] 
            )
    ...
    def analyze_label(self, label):
        ...
        if not self.enable_cc_analysis:
            ...
            for location_type in label_ids:
                numbered_ground_truth, label_cc_n = cc3d.connected_components(label == location_type, connectivity=26, return_N=True)
                print(f"Number of connected components: {label_cc_n}")
                print(f"Unique lesion id: {ID_TO_LABEL[location_type]}")
                for i in range(1, label_cc_n+1):
                    lesion_locations = numbered_ground_truth == i
                    unique_lesion_map.append({"label": lesion_locations, 'location_type' : location_type})
            ...
            if len(numbered_ground_truth) == 0:
                label_cc_sizes = 0
        return foreground_locs, label_cc_n, label_cc_sizes, unique_lesion_map
    ...
    def resample_and_normalize_case(
        ...
        for i in range(len(images)):
            try:
                images[i] = resize(images[i], output_shape=target_size, order=3, anti_aliasing=True)
            except OverflowError:
                logging.error("Unexpected values in either shape or image for resize")
        if label is not None:
            try:
                label = resize(label, output_shape=target_size, order=0, anti_aliasing=False).astype(int)    
            except OverflowError:
