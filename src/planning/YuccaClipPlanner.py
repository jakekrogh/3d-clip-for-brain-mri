...
from preprocessing.YuccaClipPreprocessor import YuccaClipPreprocessor
class YuccaClipPlanner(object):
    """
    ...
    def __init__(
        self,
        task,
        preprocessor="YuccaClipPreprocessor",
        threads=None,
    ...
    def determine_target_size_from_fixed_size_or_spacing(self):
        self.fixed_target_size = (192,192,192) #(256,256,240)
    ...
    def preprocess(self):
        ...
        #     folder=[join("src/preprocessing")],
        #     class_name=self.preprocessor,
        #     current_module="yucca.preprocessing",
        # )

        preprocessor = YuccaClipPreprocessor(self.plans_path, self.task, self.threads, self.disable_sanity_checks, self.enable_cc_analysis)
        preprocessor.run()
        self.postprocess()
